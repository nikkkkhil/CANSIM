...
* [Quick Start](#quick-start)
  ...
  * [cansim without CAN hardware](#cansim-without-can-hardware)
    * [VCAN](#vcan)
...
## Overview
Most of automotive projects need to have an access to the Controller Area Network (CAN) bus. There are plenty of commercial frameworks that provides CAN stacks and hardware/software tools necessary to develop proper CAN networks. They are very comprehensive and thus expensive. cansim aims to be cost-effective replacement for CAN simulation software. It can work with variety of CAN hardware interfaces (e.g. Microchip, Vector, PEAK-Systems) or even without it (vcan and [cannelloni](https://github.com/mguentner/cannelloni)) . cansim enables to simulate CAN signals such as ignition status, doors status or reverse gear by every automotive developer. Thanks to modularity it is easy to implement new, custom features.
...
## Build instructions
cansim project uses Travis CI (Linux) and AppVeyor (Windows) continuous integration tools. You can always check .travis.yml and .appveyor.yml files for building details.

### Linux
```
git clone https://github.com/nikkkkhil/cansim.git
cd cansim
...
```
### ARCH Linux
Install AUR package: [cansim-git](https://aur.archlinux.org/packages/cansim-git/)
...
```
## Prebuilt packages
Binary packages are automatically uploaded by CI tools (i.e. Travis and Appveyor) to [Bintray](https://bintray.com/rkollataj/cansim) artifactory for every commit on master branch.
### Download
...
### Package naming
***cansim-X.Y.ZZZZZZZ-SYS[-standalone]***
...
To run standalone version use cansim.sh script.
### Windows
...
### CAN Hardware
The list below shows hardware that has been successfuly used with cansim.
...
interface: CANUSB
```
### cansim without CAN hardware
cansim can be used without actual CAN hardware thanks to Linux's built-in emulation.
#### VCAN
Configuration:
```
sudo modprobe vcan
...
<p align="left">
<img src="https://at.projects.genivi.org/wiki/download/attachments/14976114/cansim-cannelloni.png" width="50%" />
</p>
...
1. Install libqtCannelloniCanBusPlugin.so that is built along with cansim. You can either copy it manually to Qt plugins directory (e.g. /usr/lib/qt/plugins/canbus) or use "make install" to do it automatically.
2. Create new project in cansim and add CanDevice node
...
## Help
### CAN Signals
cansim provides support for CAN signals handling. [DBC](http://socialledge.com/sjsu/index.php/DBC_Format) database description format is supported. New formats can be added via extension of [CANdb](www.github.com/GENIVI/CANdb).
...
Work on signals reception and decoding is ongoing. Experimental support is available on [signals](https://github.com/GENIVI/cansim/tree/signals) branch
...
make
```
5. Your component is now integrated with cansim
